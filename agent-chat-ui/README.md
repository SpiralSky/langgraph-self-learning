...
4. Modify the chat connection hook (`src/hooks/useChatConfig.ts` + `src/lib/chatApi.ts`) to pass your authentication token through headers to the LangGraph server:
...
This project uses the ASCII bell character (`\a`, octal `\007`, hex `\x07`) to provide audible feedback for task completion events and operation requests. The bell is emitted via stderr (`echo -e '\a' >&2`) to avoid interfering with piped stdout output. This is especially useful for long-running graph operations — the terminal bell lets you know when a task has finished without needing to watch the screen.
